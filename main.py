...
Seed = [[230, 230], [240, 230], [250, 230], [270, 230], [230, 240], [260, 250], [270, 250], [240, 260], [250, 260],
		[270, 260], [230, 270], [250, 270], [270, 270]]
		...
		for y in range(0, 500, 10):
			...
			for a in range(max(x - 10, 0), min(x + 20, 500), 10):
				for b in range(max(y - 10, 0), min(y + 20, 500), 10):
					if Cells['%i,%i' % (a, b)].alive:
						neighbor += 1
			if Cells['%i,%i' % (x, y)].alive:
				if neighbor < 3 or neighbor > 4:  # (it counts itself)
					Cells['%i,%i' % (x, y)].die()
			else:
				if neighbor == 3:
					Cells['%i,%i' % (x, y)].born()
